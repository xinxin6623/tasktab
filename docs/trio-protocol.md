...
> 进度只走这三份：「做了什么」进 CHANGELOG，「下一步」进 INDEX Handoff。**不再维护独立的非工程进度页**（v2 的 PROJECT_PROGRESS.md 已于 v3 退役；本项目已于 2026-06-14 落地退役）。
...
拆 `### 概述` + `### 明细` 两个子段。**概述**是一句话级的下一步（TaskBoard App 标签页只抓这段当 next/blocked_by）；**明细**是给人/agent 看的展开说明，App 完全不读。
...
> 概述写**纯文本加粗行**（每行一条），App 解析：非 `⚠/阻塞` 前缀归 next，`⚠`/`阻塞` 前缀归 blocked_by；列表前缀 `- ` 可选（写不写都识别），旧 `- xxx` 列表行与无 `### 概述` 的单段写法都向后兼容。
