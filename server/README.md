...
`TB_REGISTRY` 推荐填 GitHub 坐标（如 `xinxin6623/tasktab@main:server/registry.yaml`），让 registry 本身也走
单一真相；本地联调可直接填本地 registry 路径。

> ⚠️ **两份 registry 别搞混**：`cli/registry.yaml` 是 `cra` 在本机维护的、**含绝对路径、已 gitignore、不上 GitHub**；
> 服务器聚合读的是 **`server/registry.yaml`**（只留 id/name/github/pinned，无本机路径，已入库）。所以 `TB_REGISTRY`
> 的 GitHub 坐标必须指向 `server/registry.yaml`，**不是 `cli/`**（指错会报「GitHub registry 为空或不存在」）。
...
```bash
./build.sh           # 交叉编译 linux/amd64 静态二进制（重活放本地，ECS 内存紧不在上面跑编译）
